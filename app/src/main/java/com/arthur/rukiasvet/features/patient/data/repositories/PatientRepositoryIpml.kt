...
import com.arthur.rukiasvet.features.patient.data.datasources.remote.mapper.toDomain
import com.arthur.rukiasvet.features.patient.data.datasources.remote.mapper.toEntity
...
class PatientRepositoryImpl @Inject constructor(
    ...
    override suspend fun addPatient(token: String, patient: PatientRequest, imageFile: File?): Boolean {
        return try {
            ...
            val response = api.addPatient(
                token = "Bearer $token", name = namePart, species = speciesPart,
                gender = genderPart, description = descriptionPart, weight = weightPart,
                age = agePart, owner = ownerPart, telephone = telephonePart,
    ...
    override suspend fun updatePatient(token: String, id: Int, patient: PatientRequest, imageFile: File?): Boolean {
        return try {
            val namePart        = patient.name.toRequestBody("text/plain".toMediaTypeOrNull())
            val speciesPart     = patient.species.toRequestBody("text/plain".toMediaTypeOrNull())
            val genderPart      = patient.gender.toRequestBody("text/plain".toMediaTypeOrNull())
            ...
            val response = api.updatePatient(
                token = "Bearer $token", id = id, name = namePart, species = speciesPart,
                gender = genderPart, description = descriptionPart, weight = weightPart,
                age = agePart, owner = ownerPart, telephone = telephonePart,
                branchId = branchIdPart, image = imagePart
            )
    ...
    override suspend fun getPatientsByBranch(token: String, branchId: Int): List<Patient> {
        return try {
            ...
            if (response.isSuccessful) {
                ...
                patientDao.deleteByBranch(branchId)
                patientDao.insertAll(list.map { it.toEntity() })
                list
            } else {
                patientDao.getByBranch(branchId).map { it.toDomain() }
            }
        } catch (e: Exception) {
            Log.d("PatientRepo", "Sin internet, cargando desde Room")
            patientDao.getByBranch(branchId).map { it.toDomain() }
        }
    }
    ...
    override suspend fun deletePatient(token: String, id: Int): Boolean {
        return try {
            api.deletePatient("Bearer $token", id).isSuccessful
